  ...
  return <div className="blog26-page">
    ...
    <div className="blog26-workspace">
      ...
      <aside className="blog26-side card">
        <div className="blog26-side-tabs">
          <button className={activeSide==='photos'?'active':''} onClick={()=>setActiveSide('photos')}>📷 사진</button>
          <button className={activeSide==='seo'?'active':''} onClick={()=>setActiveSide('seo')}>SEO</button>
          <button className={activeSide==='compliance'?'active':''} onClick={()=>setActiveSide('compliance')}>규정</button>
        </div>
        {activeSide==='photos'&&<PhotoPanel assets={advertiserAssets} project={project} onAttach={attachAsset} onRegister={()=>setAssetOpen(true)} onDelete={async id=>{await blogApi.deleteAsset(id);setAssets(rows=>rows.filter(r=>r.assetId!==id));}}/>}
    ...
    {assetOpen&&<AssetModal advertiserId={project.advertiserId} assets={advertiserAssets} imageBlocks={project.blocks.filter(b=>b.type==='image')} onClose={()=>setAssetOpen(false)} onAttach={(blockId,assetId)=>{attachAsset(blockId,assetId);setAssetOpen(false)}} onAdded={a=>setAssets(rows=>[a,...rows])} onDeleted={id=>setAssets(rows=>rows.filter(r=>r.assetId!==id))}/>}
...
function PhotoPanel({assets,project,onAttach,onRegister,onDelete}:{assets:BlogAsset[];project:BlogProject;onAttach:(blockId:string,assetId:string)=>void;onRegister:()=>void;onDelete:(assetId:string)=>void}){const imageBlocks=project.blocks.filter(b=>b.type==='image');return <section className="blog26-side-section"><div className="blog26-side-head"><div><b>사진 라이브러리</b><span>AI 생성 시 자동으로 사진 위치를 배정합니다</span></div><button className="btn secondary mini" onClick={onRegister}><Plus size={13}/> 사진 등록</button></div>{!imageBlocks.length&&<div className="blog26-clean">본문에 이미지 블록을 먼저 추가하세요.</div>}<div className="blog26-photo-list">{assets.map(a=><article key={a.assetId}><div className="blog26-photo-thumb">{a.url?<img src={a.url} alt=""/>:<ImageIcon size={22}/>}</div><div><b>{a.name}</b><small>{a.tags.join(', ')||'태그 없음'}</small>{a.caption&&<small style={{color:'#5a6a83'}}>{a.caption}</small>}</div><div style={{display:'flex',gap:6,alignItems:'center'}}>{imageBlocks.length>0&&<select defaultValue="" onChange={e=>e.target.value&&onAttach(e.target.value,a.assetId)}><option value="">위치 선택</option>{imageBlocks.map((b,i)=><option key={b.blockId} value={b.blockId}>사진 {i+1}</option>)}</select>}<button className="icon-btn" style={{color:'#dc2626',border:'none',background:'transparent',cursor:'pointer',fontSize:14}} title="삭제" onClick={()=>onDelete(a.assetId)}>×</button></div></article>)}{!assets.length&&<div className="blog26-list-empty small"><ImageIcon size={24}/><b>등록된 사진이 없습니다.</b><span>사진을 등록하면 AI 글 생성 시 자동으로 배정됩니다.</span></div>}</div></section>}
...
function AssetModal({advertiserId,assets,imageBlocks,onClose,onAttach,onAdded,onDeleted}:{advertiserId:string;assets:BlogAsset[];imageBlocks:BlogBlock[];onClose:()=>void;onAttach:(blockId:string,assetId:string)=>void;onAdded:(a:BlogAsset)=>void;onDeleted:(id:string)=>void}){
  const [tab,setTab]=useState<'upload'|'url'>('upload');
  const [name,setName]=useState('');const [url,setUrl]=useState('');const [tags,setTags]=useState('');const [caption,setCaption]=useState('');
  const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState('');const [uploading,setUploading]=useState(false);const [notice,setNotice]=useState('');
  const pickFile=(f:File|null)=>{setFile(f);if(f){setPreview(URL.createObjectURL(f));if(!name)setName(f.name.replace(/\.[^.]+$/,''));}else{setPreview('');}};
  const addByFile=async(e:FormEvent)=>{e.preventDefault();if(!file)return;setUploading(true);setNotice('');try{const fd=new FormData();fd.append('file',file);fd.append('advertiserId',advertiserId);fd.append('name',name||file.name);fd.append('tags',tags);fd.append('caption',caption);const row=await blogApi.uploadAsset(fd);onAdded(row);setFile(null);setPreview('');setName('');setTags('');setCaption('');setNotice('등록 완료!');}catch(e){setNotice(e instanceof Error?e.message:'업로드 실패');}finally{setUploading(false);}};
  const addByUrl=async(e:FormEvent)=>{e.preventDefault();setUploading(true);setNotice('');try{const row=await blogApi.addAsset({advertiserId,name,url,tags:split(tags),caption});onAdded(row);setName('');setUrl('');setTags('');setCaption('');setNotice('등록 완료!');}catch(e){setNotice(e instanceof Error?e.message:'등록 실패');}finally{setUploading(false);}};
  const del=async(assetId:string)=>{if(!confirm('이 사진을 삭제할까요?'))return;await blogApi.deleteAsset(assetId);onDeleted(assetId);};
  return <div className="modal-backdrop" onClick={onClose}><div className="modal-card blog26-modal" style={{maxWidth:560}} onClick={e=>e.stopPropagation()}>
    <div className="modal-head"><div><h3>사진 등록 / 관리</h3><p>등록된 사진은 AI 글 생성 시 자동으로 어울리는 위치에 배정됩니다.</p></div><button className="icon-btn" onClick={onClose}>×</button></div>
    ...
    {assets.length>0&&<><div style={{borderTop:'1px solid #edf0f4',margin:'16px 0 12px',paddingTop:12,fontWeight:700,fontSize:13}}>등록된 사진 ({assets.length}장)</div>
    <div className="blog26-asset-picker">{assets.map(a=><article key={a.assetId}>
      <div>{a.url?<img src={a.url} alt=""/>:<ImageIcon size={22}/>}</div>
      <div style={{flex:1,minWidth:0}}><b style={{display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.name}</b><small style={{color:'#8592a6'}}>{a.tags.join(', ')||'태그 없음'}</small>{a.caption&&<small style={{display:'block',color:'#5a6a83'}}>{a.caption}</small>}</div>
      <div style={{display:'flex',gap:6,flexShrink:0}}>
        {imageBlocks.length>0&&<select defaultValue="" onChange={e=>e.target.value&&onAttach(e.target.value,a.assetId)}><option value="">본문에 연결</option>{imageBlocks.map((b,i)=><option key={b.blockId} value={b.blockId}>사진 {i+1}</option>)}</select>}
        <button style={{border:'none',background:'none',color:'#dc2626',cursor:'pointer',fontSize:16,lineHeight:1}} onClick={()=>del(a.assetId)} title="삭제">×</button>
      </div>
    </article>)}</div></>}
