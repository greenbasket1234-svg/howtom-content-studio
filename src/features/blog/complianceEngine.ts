...
export function analyzeCompliance(project: BlogProject): ComplianceIssue[] {
  ...
  if (isMedicalIndustry(project.industry)) {
    issues.push({ id: 'medical-note', severity: 'info', category: '의료광고 안내', phrase: '사전심의·부작용 안내', reason: '의료 업종은 매체·내용에 따라 사전심의 대상 여부와 부작용 안내 필요 여부가 달라질 수 있습니다.', suggestion: '발행 전 담당자가 별도로 확인해주세요.' });
  }
  return issues;
}
