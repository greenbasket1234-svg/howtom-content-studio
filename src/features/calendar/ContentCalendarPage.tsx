...
type CalendarItem = { id: string; kind: '광고' | '블로그' | '영상 대본' | '문서'; title: string; advertiserId: string; date: string; openRoute: string };
...
export function ContentCalendarPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setItems([
        ...ads.map(p => ({ id: p.projectId, kind: '광고' as const, title: p.title, advertiserId: p.advertiserId, date: p.updatedAt, openRoute: `/production/ad?project=${p.projectId}` })),
        ...blogs.map(p => ({ id: p.projectId, kind: '블로그' as const, title: p.selectedTitle || p.primaryKeyword || '제목 미정', advertiserId: p.advertiserId, date: p.updatedAt, openRoute: `/production/blog?project=${p.projectId}` })),
        ...docs.map(p => ({ id: p.projectId, kind: '문서' as const, title: p.title, advertiserId: p.advertiserId, date: p.updatedAt, openRoute: `/production/document?project=${p.projectId}` })),
        ...scripts.map(p => ({ id: p.projectId, kind: '영상 대본' as const, title: p.title, advertiserId: p.advertiserId, date: p.updatedAt, openRoute: `/production/video-script?project=${p.projectId}` })),
      ]);
      setLoading(false);
    })();
  }, []);
  ...
  const byDay = useMemo(() => {
    ...
    for (const item of visible) {
      ...
      map.set(d.getDate(), [...(map.get(d.getDate()) || []), item]);
    }
    return map;
  }, [visible, year, month]);
